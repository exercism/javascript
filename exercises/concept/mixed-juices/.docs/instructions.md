...
Dmitry will then make the remaining juices.

To help with the hand-over, implement a function `remainingOrders` which takes the number of minutes left in Li Mei's shift and an array of juices that have been ordered but not prepared yet.
...
If Li Mei starts to mix a certain juice, she will always finish the job even if she has to work a bit longer.
The time left in the shift will always be greater than 0, so there will always be at least one juice that Li Mei will prepare.
