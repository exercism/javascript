...
describe('front door response', () => {
  ...
  xtest('should take the first character of each line', () => {
  ...
  xtest('should ignore whitespace when taking first characters', () => {
    const expectedLetters = ['S', 'H', 'I', 'R', 'E'];
    expect(recite(SHIRE_HORSE_WITH_SPACES, frontDoorResponse)).toStrictEqual(
      expectedLetters
    );
  });
});
...
describe('back door password', () => {
  xtest("should generate the correct pass phrase from 'horse'", () => {
    expect(backDoorPassword('horse')).toBe('Horse, please');
  });

  xtest("should generate the correct pass phrase from 'work'", () => {
