import { largestSeriesProduct } from './largest-series-product';

describe('Series', () => {
  test('can get the largest product of 2', () => {
    expect(largestSeriesProduct('0123456789', 2)).toBe(72);
  });

  xtest('works for a tiny number', () => {
    expect(largestSeriesProduct('19', 2)).toBe(9);
  });

  xtest('can get the largest product of 3', () => {
    expect(largestSeriesProduct('1027839564', 3)).toBe(270);
  });
  ...
  xtest('can get the largest product of a big number', () => {
    ...
    expect(largestSeriesProduct(largeNumber, 13)).toBe(23514624000);
  });

  xtest('returns 0 if all digits are zero', () => {
    expect(largestSeriesProduct('0000', 2)).toBe(0);
  });

  xtest('returns 0 if all spans contain zero', () => {
    expect(largestSeriesProduct('99099', 3)).toBe(0);
  });
  ...
  xtest('returns 1 for empty string and zero slice length', () => {
    expect(largestSeriesProduct('', 0)).toBe(1);
  });

  xtest('returns 1 for non-empty string and zero slice length', () => {
    expect(largestSeriesProduct('123', 0)).toBe(1);
  });
